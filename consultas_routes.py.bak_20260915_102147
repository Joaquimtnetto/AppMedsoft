...
@consultas_bp.route('/api/consultas-paciente/itens/<int:consulta_id>', methods=['PUT'])
def alterar_consulta_paciente(consulta_id):
    data = request.get_json(silent=True) or {}
    try:
        codpac = int(data.get('codpac'))
        dtvisita = date.fromisoformat(str(data.get('dtvisita') or ''))
    except (TypeError, ValueError):
        return jsonify({'success': False, 'message': 'Consulta ou data inválida.'}), 400
    historico = str(data.get('historico') or '').strip()
    if not historico:
        return jsonify({'success': False, 'message': 'Informe o histórico da consulta.'}), 400
    ...
    try:
        ...
        cur.execute('''
            UPDATE public.consulta
               SET dtvisita = %s, historico = %s, datatualiza = CURRENT_TIMESTAMP
             WHERE cod = %s AND codpac = %s AND codclin = %s
         RETURNING cod
        ''', (dtvisita, historico.encode('utf-8'), consulta_id, codpac, company_id))
        if not cur.fetchone():
            return jsonify({'success': False, 'message': 'Histórico não encontrado.'}), 404
        cur.execute('''
            UPDATE public.pacient
        ...
        con.commit()
        return jsonify({'success': True, 'message': 'Histórico alterado com sucesso.'})
    except DBConnectionError as exc:
        return jsonify({'success': False, 'message': str(exc)}), 503
    except Exception:
        if con:
            con.rollback()
        return jsonify({'success': False, 'message': 'Erro interno ao alterar o histórico.'}), 500
    finally:
        if con:
            con.close()
