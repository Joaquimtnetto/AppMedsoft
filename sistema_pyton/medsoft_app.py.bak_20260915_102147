...
from flask import Flask, current_app, request, jsonify, render_template, session, redirect, url_for
...
if configured_build_datetime:
    ...
    BUILD_ENVIRONMENT = 'Desenvolvimento'
...
def current_build_metadata():
    ...
    source_roots = [
        ...
        project_root / 'static',
        project_root / 'whatsapp-baileys' / 'src',
    ]
    source_files = [project_root / 'whatsapp-baileys' / 'package.json']
    allowed_suffixes = {'.py', '.html', '.js', '.css', '.json'}
    ignored_parts = {'node_modules', 'sessions', 'message-history', 'logs', '__pycache__'}
    ...
    timestamps = [path.stat().st_mtime for path in source_files if path.is_file()]
    latest = datetime.datetime.fromtimestamp(max(timestamps)).astimezone() if timestamps else datetime.datetime.now().astimezone()
    return latest.strftime('%d/%m/%Y %H:%M'), latest.strftime('%Y%m%d%H%M%S'), BUILD_ENVIRONMENT
...
def _windows_desktop_path():
    ...
    if sys.platform == 'win32':
        try:
            ...
            resolved = Path(os.path.expandvars(desktop)).expanduser()
            if resolved.is_dir():
                return resolved
...
def _install_medsoft_shortcut_icon():
    ...
    installed_icon = icon_directory / 'medsoft.ico'
    source_icon = Path(resource_path(os.path.join('static', 'medsoft.ico')))
    installed_icon.write_bytes(source_icon.read_bytes())
    return installed_icon


@app.route('/api/criar-atalho-medsoft', methods=['POST'])
def criar_atalho_medsoft():
    try:
        desktop = _windows_desktop_path()
        desktop.mkdir(parents=True, exist_ok=True)
        icon = _install_medsoft_shortcut_icon()
        shortcut = desktop / 'MedSoft.url'
        shortcut.write_text(
            '[InternetShortcut]\n'
            'URL=https://sistema.medsoft.com.br\n'
            f'IconFile={icon}\n'
            'IconIndex=0\n',
            encoding='utf-8',
        )
