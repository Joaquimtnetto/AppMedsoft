(function () {
    ...
    function init() {
        ...
        function updateHistoryRangeFields() {
            var preset = document.getElementById('crm-history-range-preset').value;
            page.querySelectorAll('.crm-history-custom').forEach(function(field) {
        ...
        function historyRangeQuery() {
            var preset = document.getElementById('crm-history-range-preset').value;
            if (preset === 'all') return 'todo_historico=1';
            var finalDate = new Date();
            var initialDate = new Date(finalDate.getFullYear(), finalDate.getMonth(), finalDate.getDate());
            if (preset === 'custom') {
                var from = document.getElementById('crm-history-date-from').value;
                var to = document.getElementById('crm-history-date-to').value;
                if (!from || !to) throw new Error('Informe a data inicial e a data final.');
                if (from > to) throw new Error('A data inicial não pode ser posterior à data final.');
                return 'data_inicial=' + encodeURIComponent(from) + '&data_final=' + encodeURIComponent(to);
            }
            initialDate.setDate(initialDate.getDate() - Math.max(0, Number(preset || 30) - 1));
            return 'data_inicial=' + encodeURIComponent(isoDate(initialDate)) +
                '&data_final=' + encodeURIComponent(isoDate(finalDate));
        }

        function setSyncStatus(text, state) {
            var status = document.getElementById('crm-sync-status');
            var label = document.getElementById('crm-sync-status-text');
        ...
        async function loadConnection() {
            ...
            try {
                ...
                badge.innerHTML = status.connected
                    ? '<i class="fa fa-phone"></i> ' + CrudUI.escapeHtml(status.syncingHistory
                        ? 'Sincronizando mensagens...'
                        : (status.phone || 'WhatsApp conectado'))
                    : '<i class="fa fa-circle-exclamation"></i> ' + CrudUI.escapeHtml(
                        status.status === 'qr' ? 'Aguardando leitura do QR' : 'WhatsApp desconectado');
            } catch (error) {
        ...
        async function loadContacts() {
            ...
            try {
                ...
                setSyncStatus('Não foi possível atualizar os contatos.', 'error');
            } finally {
                if (button) button.disabled = false;
            }
        }

        async function renderContacts(contacts, showProgress) {
            var container = document.getElementById('crm-whatsapp-contacts-list');
            ...
            if (!contacts.length) {
                container.innerHTML = '<div class="crud-empty">Nenhum contato sincronizado pelo WhatsApp. Clique em Atualizar contatos.</div>';
                if (showProgress) setSyncStatus('Contatos atualizados: 0/0.', 'success');
                return;
            }
            container.innerHTML = '';
            for (var start = 0; start < contacts.length; start += 100) {
                var end = Math.min(start + 100, contacts.length);
                container.insertAdjacentHTML('beforeend', contacts.slice(start, end).map(function(item) {
                ...
                if (showProgress) setSyncStatus('Atualizando contatos (' + end + '/' + contacts.length + ') — aguarde...', 'working');
                await new Promise(function(resolve) { window.requestAnimationFrame(resolve); });
            }
            if (showProgress) setSyncStatus('Contatos atualizados: ' + contacts.length + '/' + contacts.length + '.', 'success');
        }
        ...
        async function load() {
            loadConnection();
            setSyncStatus('Carregando conversas — aguarde...', 'working');
            ...
            try {
                ...
                renderConversations();
                setSyncStatus('Conversas carregadas. Atualizando mensagens — aguarde...', 'working');
                ...
                setSyncStatus('Não foi possível carregar as conversas.', 'error');
            }
            var rangeQuery;
            try {
                rangeQuery = historyRangeQuery();
            } catch (error) {
                setSyncStatus(error.message, 'error');
                return;
            }
            CrudUI.request('/api/whatsapp/received?' + rangeQuery, {method: 'GET', headers: headers()})
                .then(async function (syncResult) {
                    var refreshed = await CrudUI.request('/api/crm/whatsapp/conversas', {method: 'GET', headers: headers()});
                    conversations = refreshed.conversas || [];
                    renderConversations();
                    var sync = syncResult.sync || {};
                    var total = Number(sync.total || 0);
                    setSyncStatus('Período importado: ' + total + ' mensagens encontradas (' + Number(sync.importadas || 0) + ' novas).', 'success');
                })
                .catch(function () { setSyncStatus('Falha ao atualizar mensagens. Tente novamente.', 'error'); });
        }
        ...
        document.getElementById('crm-history-range-preset').onchange = updateHistoryRangeFields;
